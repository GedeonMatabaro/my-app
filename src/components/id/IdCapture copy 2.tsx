...
export default function IdCapture({
  userId, kind, deviceId, country, onUploaded
}: {
  userId: string;
  kind: IdKind;
  ...
  const tick = () => {
    // If preview is open, don't keep processing frames
    ...
    // Loosened gates (~80% of strictness)
  ...
  const startCam = async () => {
    if (previewUrl) return; // don't start if previewing
    cancelRAF();
  ...
  // Start/stop with device/kind/preview state
  React.useEffect(() => {
    (async () => {
      if (previewUrl) {
        stopCam();                 // close camera while previewing
      } else {
        await startCam();          // (re)open when not previewing
      }
    })().catch(console.error);

    return () => stopCam();        // cleanup on unmount
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [deviceId, kind, previewUrl]);
  ...
  function closePreview(){
    ...
    setPreviewUrl(null);    // <- effect will restart camera
  }

  async function confirmUpload(){
    if(!previewUrl) return; setBusy(true);
    try{
      ...
      closePreview(); // will restart camera
    ...
    <div className="grid gap-3">
      ...
      {previewUrl && (
        <div className="fixed inset-0 z-[999] bg-black/60 grid place-items-center p-4">
          <div className="bg-white dark:bg-neutral-900 rounded-2xl shadow-2xl max-w-[92vw] w-[680px] p-4 grid gap-3">
            <div className="text-sm font-medium">Preview (cropped to frame)</div>
            <img src={previewUrl} alt="Preview" className="rounded-lg max-h-[70vh] object-contain" />
      ...
      {uploadedUrl && (
        <div className="text-sm">
          ...
          <a className="text-blue-600 underline break-all" href={uploadedUrl} target="_blank" rel="noreferrer">
            {uploadedUrl}
          </a>
        </div>
      )}
