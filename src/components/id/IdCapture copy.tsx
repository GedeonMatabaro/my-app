...
export default function IdCapture({ userId, kind, deviceId, country, onUploaded }: {
  userId: string;
  kind: IdKind;
  ...
  function computeOverlayWidth() {
    ...
    if (!vw) return;
    // fill width minus 15px padding on each side (30 total); try to keep >= 300px if space allows.
  ...
  const tick = () => {
    ...
    if (!v || !c || !f || !v.videoWidth) return requestAnimationFrame(tick);

    // draw full frame
    c.width = v.videoWidth; c.height = v.videoHeight;
    const ctx = c.getContext('2d')!; ctx.drawImage(v, 0, 0, c.width, c.height);

    // compute crop rect in VIDEO pixels based on DOM overlay box
    ...
    // loosened gates ~80% of original strictness
  ...
  async function onCapture() {
    ...
    // crop into a new canvas the framed part only
